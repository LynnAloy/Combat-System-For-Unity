...
public class MeeleFighter : MonoBehaviour
{
    ...
    {
        if (sword == null || sword.activeSelf == visible)
        {
            return;
        }
    ...
    private IEnumerator Attack(MeeleFighter target = null)
    {
        ...
        { 
            ...
            float animationDuration = Mathf.Max(animatorState.length, NumericGuard.MinDenominator);
            float normalizedTime = Mathf.Clamp01(timer / animationDuration);
            if (target != null && attack.MoveToTarget)
            {
                float moveTiming = Mathf.InverseLerp(attack.MoveStartTime, attack.MoveEndTime, normalizedTime);
